...
the TEEP Agent plus TA-local protected state authorize a Catalog and then an
app. Success at one stage does not grant the authority of
the next. In particular, the TrustZone REE provides availability, HTTP/TLS and
byte transport, and cache access. It does not
...
`insecure_demo_mode` may be enabled only with `attestam-insecure`. Under D045, `attestam-verified` is also an explicitly insecure PoC protocol path because it uses fixed development TEEP Agent and Generic EAT signers; it must retain `final-verified=false` and is not a production trust claim. It rejects unverifiable TEEP messages, unsigned components, components with SUIT mismatches, and apps not authorized by the protected Catalog. The feature-gated historical M9.1 smoke still stops before Catalog/app promotion or general app execution; the default M9.3 path permits only the bounded protected single-app flow described above.

TEEP_Agent Wasm itself is the root-side component that validates, obtains, and installs the Catalog File and Trusted Wasm Apps. In the plain Linux PoC, it is installed from a repository build artifact into the state directory, but privileged TEEP_Agent hostcall capability is granted only after `twep-wr` verifies the embedded `twep.sig` code signature with the insecure demo TEEP Agent code-signing public key. General Trusted Wasm Apps must verify with the separate insecure demo app code-signing public key and are rejected if signed as a TEEP Agent. TEEP Agent privilege therefore derives from the code-signing identity, not from the state-directory file path alone. The TrustZone TA enforces the same derivation independently: it parses the final `twep.sig` section, requires `alg="ESP256"` and `role="teep-agent"`, and verifies the SHA-256/P-256 signature with its embedded demo TEEP Agent public key before WAMR initialization, native-hostcall registration, or module loading. Missing, malformed, role-mismatched, or cryptographically invalid signatures fail with `TEE_ERROR_SECURITY`. The D045 TrustZone PoC additionally checks the TA-local TEEP_Agent measurement but does not claim a production root of trust; the embedded public key and corresponding build-side demo private key remain public development credentials. Updating TEEP_Agent itself through AttesTAM is outside the PoC scope, avoiding circular trust in which TEEP_Agent updates itself in the same TEEP session.
...
Evidence is backend-selected. Linux and OP-TEE use Generic EAT constructed and signed inside the Rust TEEP_Agent with the fixed development ES256 Evidence key; AttesTAM forwards it to Veraison. SGX hardware creates the canonical DCAP Quote3 bundle inside the Enclave, binding the QueryRequest challenge and Agent public COSE_Key, and Intel QVL appraises the Quote before AttesTAM acceptance. SDK Simulation and fake QE behavior are confined to the private backend-test harness. In every case, TEEP_Agent learns AttesTAM acceptance only from a TAM-signed Update bound to the live transcript. Repository credentials remain public development fixtures, so successful Generic EAT or Quote3 appraisal does not establish `final-verified=true`.

The Rust TEEP Agent also owns the fixed development QueryResponse/Success
signer. No backend supplies an Agent public-key or ESP256 signing hostcall, and
SGX no longer provisions a separate sealed Agent key. Pre-existing SGX
`agent-key.blob` files are ignored and remain outside the active trust boundary.
