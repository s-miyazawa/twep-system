...
The Evidence function inside TEEP_Agent handles only the TEEP-common `challenge`, the TEEP Agent public COSE_Key, and the output buffer. The current Generic EAT format is authoritative for the Evidence payload because AttesTAM forwards that Evidence to Veraison. TEEP_Agent does not consume the Veraison result. Evidence derived from Intel SGX DCAP Quotes, OP-TEE/TrustZone tokens, hardware-specific keys, or production credential services is outside this PoC. In `attestam-insecure` and the D045 `attestam-verified` PoC path, the Rust TEEP_Agent constructs the development Generic EAT and signs it with its separate fixed ES256 Evidence key. The private key is publicly embedded in Wasm and forgeable, so this proves protocol interoperability but cannot advance to `final-verified=true`. The legacy `create_evidence` hostcall remains available for ABI and diagnostic compatibility; normal sessions do not call it, and an unset callback returns `unsupported`.
