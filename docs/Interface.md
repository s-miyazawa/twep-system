...
- Callbacks registered with `twep_wr_set_host_io` must remain valid until `twep_wr_shutdown`. HTTP communication is implemented by TEEP_Broker; the C side performs only policy checks and buffer copies. The development mock Evidence for `attestam-insecure` is also generated by a TEEP_Broker callback, but this is not the final trust boundary. Production Evidence generation will move to the TEEP_Agent inside the TEE and platform-specific TEE/Enclave implementations.
...
The `-o` destination may be any path writable with user privileges. Because twepd runs as a user service in the REE for both primary backends, whether output can be saved is governed by normal user privileges and host-side path validation. A system-service deployment must define a separate output policy.
...
The protected credential store used by the D045 TrustZone PoC is distinct from `dev-trust-anchors.cbor` above. The TrustZone backend stores the fixed public-key and policy fixtures in OP-TEE REE FS Secure Storage. `CFG_REE_FS=y` and `CFG_RPMB_FS=n` are permanent settings, and rollback attacks are outside this repository's threat model. Files in the REE state directory and file-backed sealed objects under `platform/linux` remain observation fixtures rather than protected TrustZone state.

Initial population is performed by the repository fixture generator and TrustZone smoke `provision` commands, not through TEEP Update. No credential issuance, developer/device enrollment, rotation, revocation, recovery, or remote provisioning service is part of this PoC. Platform-dependent implementations on the `twep-wr` side are separated into `lib/twep-wr/src/platform/<backend>/`. The initial backend names are `linux`, `trustzone`, `sgx`, and `keystone`; SGX and Keystone remain portability boundaries rather than implemented credential services. `platform/linux` is an REE-only development backend that places file-backed objects at `$STATE/platform/linux/sealed/<object-name>`. The protected object names are `protected-credential-store.cbor`, `protected-issuer-allowlist.cbor`, `protected-store-freshness.cbor`, `protected-revocation-state.cbor`, `protected-agent-identity.cbor`, `teep-acceptance-state.cbor`, and the compatibility name `verified-evidence-result.cbor` for the AttesTAM acceptance result until the persistence name is migrated. The acceptance record's two physical slot names are TA-internal and are not exposed through the generic sealed-storage interface.
...
This schema is the minimal public-key and policy-fixture contract used by OP-TEE and retained as a portability shape for future sealed-storage backends. The current fixed TEEP Agent private signer is compiled into the Rust PoC and the development Evidence signer is REE-produced; `private_key_ref` is reserved for a future product that chooses to add a protected private-key service and is not required here.
...
The abstraction function inside TEEP_Agent handles only the TEEP-common `challenge`, the TEEP Agent public COSE_Key, and the output buffer. The current Generic EAT format is authoritative for the Evidence payload because AttesTAM forwards that Evidence to Veraison. TEEP_Agent does not consume the Veraison result. Evidence derived from Intel SGX DCAP Quotes, OP-TEE/TrustZone tokens, hardware-specific keys, or production credential services is outside this PoC. In `attestam-insecure` and the D045 `attestam-verified` PoC path, a development mock EAT is generated by a TEEP_Broker callback. This proves protocol interoperability but cannot advance to `final-verified=true`. An unimplemented platform returns `unsupported`.
...
The response body of `200 OK` is treated as a COSE-wrapped TEEP message (CBOR). `204 No Content` is treated as normal session termination. At this stage, TEEP_Agent extracts the COSE_Sign1 payload and determines whether the TEEP message type is QueryRequest. If QueryRequest contains a challenge, `twep_host_create_evidence` generates a development mock EAT COSE_Sign1, which is placed in the QueryResponse `attestation-payload`. For a QueryRequest with a token, TEEP_Agent creates a QueryResponse payload containing a `twep-app-v1` SUIT Component Identifier generated from the target command. In the development insecure demo, the development ESP256 signer inside the Rust TEEP_Agent signs the payload as COSE_Sign1 before the second POST. On a platform that does not support Evidence, processing stops with `teep.attestation_unsupported` and does not proceed to a signed QueryResponse or the second POST. If the second POST returns a QueryRequest with a challenge, TEEP_Agent creates a QueryResponse containing mock EAT Evidence, saves it to `teep-agent/last-attestation-query-response.cose`, and performs a third POST. The status of the challenge-response POST is saved to `teep-agent/last-attestation-query-response-status.txt`; if a body is present, it is saved to `teep-agent/last-attestation-query-response-body.cose`.
