...
The OP-TEE backend is split into explicit `arm-optee` and `riscv-optee`
...
| --- | --- | --- | --- | --- |
| D027 | Implemented: OP-TEE host I/O uses an explicit, session-bound continuation; no WAMR call frame survives an REE round trip. | `docs/Architecture.md`, `docs/ABI.md` | `optee/twep-wr-ta/ta/`, `lib/twep-wr/src/platform/optee-common/` | host-I/O resume and negative OP-TEE smokes in `docs/Testing.md` |
...
| D045 | Implemented PoC boundary: fixed development credentials and Generic EAT Evidence signed by a fixed ES256 key inside the Rust TEEP_Agent can exercise verified protocol checks but can never establish `final-verified=true`. The private key is publicly embedded in Wasm and forgeable. | `docs/Security.md`, `docs/AttesTAM.md` | `wasm/teep-agent/`, protected credential fixtures | verified acceptance smoke and credential/policy negative smokes |
...
| D047 | Implemented: only the default-name Catalog TC can be published through the TA-owned two-slot Catalog plus D043 transaction. | `docs/ABI.md`, `docs/Security.md` | TEEP Agent Catalog validation, TA protected Catalog state | Catalog commit/readback, fault, restart, hash, resource, and object-access smokes |
| M8.5 | Complete: both production OP-TEE profiles verify the role-specific TEEP Agent code signature, then execute TEEP Agent and general applications in TA-local WAMR with public C ABI v3 transport. | `docs/Architecture.md`, `docs/Interface.md` | `lib/twep-wr/`, `optee/twep-wr-ta/` | baseline, TEEP Agent signature negative, failure, public-ABI-v3, and app execution smokes |
| M9.1 | Complete historical boundary: a dedicated smoke-only TEEP Agent build can advance D043 once, then stops without Catalog/app installation, Success, or app execution. The default TEEP Agent implements M9.3. | `docs/Interface.md`, `docs/Security.md`, `docs/Testing.md` | feature-gated M9.1 live acceptance coordinator and TA acceptance state | `smoke-optee-trustzone-attestam-verified-acceptance` |
| M9.2 | Complete historical Catalog-only boundary: validates and protects the default Catalog, sends Success after readback, then stops with `teep.verified_required`. | `docs/ABI.md`, `docs/Security.md`, `Spec.md` | TEEP Agent Catalog coordinator and TA D047 storage | `smoke-optee-trustzone-attestam-verified-catalog` and Catalog negative/fault smokes |
