...
The TEEP_Agent Wasm Application and general Trusted Wasm Applications must be distributed and executed as the same platform-backend-independent Wasm binaries. Do not produce separate Wasm artifacts for backends such as Linux, TrustZone, SGX, or Keystone. Accommodate platform differences in hostcall implementations, the C ABI/TA commands, protected storage, Evidence generation, and runtime resource policies. Do not introduce platform-specific compile-time branches into Wasm binaries.
...
The final verified path has no direct-Verifier result consumed by TEEP_Agent. Veraison's Relying Party is AttesTAM, and AttesTAM decides whether the Evidence makes the TEEP Agent acceptable. TEEP_Agent only observes that decision when AttesTAM returns a non-empty TAM-signed TEEP Update that TEEP_Agent can verify against its AttesTAM trust anchor in the live session following the Evidence-bearing QueryResponse. The legacy schema version 1 and `direct-verifier` source are retained only as dry-run compatibility observations and are not final-capable. HTTP `204 No Content` is never a valid positive source.

For `attestam-signed-update`, `challenge_response_bound=true` means the TAM-signed Update was accepted in the active challenge-response session after the Evidence-bearing QueryResponse and passed the TEEP_Agent's TAM signature, D046 rolling-token, Update, SUIT, and D043 acceptance checks. In the live protocol, the non-empty bounded token echoed from the initial QueryRequest and the non-empty bounded token in the immediate signed Update have different protocol roles and need not be byte-equal. Binding comes from the ordered continuation and the exact session-owned Evidence QueryResponse consumed by D043. The Linux fixed-input dry-run retains byte equality with `teep-agent/verified-expected-token.bin`. No additional COSE extension is required; the implementation follows the current AttesTAM server behavior.
...
- `create_evidence` is TEEP_Agent-only. It takes the `challenge` from the AttesTAM QueryRequest and the TEEP Agent public COSE_Key as input, and returns the `attestation-payload` bytes received by AttesTAM. The returned payload is a COSE_Sign1 whose payload is Generic EAT CBOR. Integration with the Veraison Generic EAT verifier takes priority; an Intel SGX DCAP Quote, OP-TEE/TrustZone token, hardware-specific evidence key, and production credential service are outside this PoC. In `attestam-insecure` and the D045 `attestam-verified` PoC path, the development mock EAT is generated through the TEEP_Broker callback. This REE producer is explicitly insecure and cannot establish `final-verified=true`. Return `8` (unsupported) on unimplemented platforms.
