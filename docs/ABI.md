...
The TEEP_Agent Wasm Application and general Trusted Wasm Applications must be distributed and executed as the same platform-backend-independent Wasm binaries. Do not produce separate Wasm artifacts for backends such as Linux, TrustZone, SGX, or Keystone. Accommodate platform differences in hostcall implementations, the C ABI/TA commands, protected storage, and runtime resource policies. The current Generic EAT development Evidence is generated by the same Rust TEEP_Agent binary on every backend. Do not introduce platform-specific compile-time branches into Wasm binaries.
...
- In `attestam-insecure` and the D045 `attestam-verified` PoC path, the Rust TEEP_Agent constructs the Generic EAT and signs its tagged COSE_Sign1 with a fixed development ES256 Evidence key embedded in the Wasm binary. The key is separate from the ESP256 TEEP message signer. Because its private key is publicly embedded and forgeable, it is neither hardware-rooted Evidence nor a production credential and cannot establish `final-verified=true`. The public `create_evidence` hostcall remains TEEP_Agent-only for ABI v3 and diagnostic compatibility, but normal sessions do not call it or produce an Evidence `need_host_io` continuation. It still accepts the `challenge` and TEEP Agent public COSE_Key and returns `8` (unsupported) when no callback is installed. Intel SGX DCAP Quotes, OP-TEE/TrustZone tokens, hardware-specific Evidence keys, and production credential services are outside this PoC.
