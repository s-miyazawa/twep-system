...
twep_wr_status_t twep_optee_read_wrapped_error_artifacts(
    ...
    size_t *out_app_wasm_len)
{
    ...
    *out_app_wasm_len = 0;
    ...
    status = twep_optee_read_teep_agent_wasm(ctx, out_teep_agent_wasm, out_teep_agent_wasm_len);
    if (status != TWEP_WR_OK) {
        return status;
    }
    if (!twep_wr_state_path(ctx, "catalog", "catalog.cbor", path, sizeof(path))) {
        twep_optee_free_artifacts(*out_teep_agent_wasm, NULL, NULL);
        *out_teep_agent_wasm = NULL;
    ...
    *out_catalog = twep_wr_read_file(path, out_catalog_len);
    ...
    if (!twep_wr_state_path(ctx, "apps", "helloworld.wasm", path, sizeof(path))) {
        twep_optee_free_artifacts(*out_teep_agent_wasm, *out_catalog, NULL);
        *out_teep_agent_wasm = NULL;
        *out_catalog = NULL;
...
twep_wr_status_t twep_optee_read_resolved_app_artifacts(
    const twep_wr_context_t *ctx,
...
twep_wr_status_t twep_optee_read_verified_acceptance_artifacts(
    const twep_wr_context_t *ctx,
    uint8_t **out_teep_agent_wasm,
    size_t *out_teep_agent_wasm_len)
{
    ...
    *out_teep_agent_wasm_len = 0;
    ...
    return twep_optee_read_teep_agent_wasm(ctx, out_teep_agent_wasm, out_teep_agent_wasm_len);
}
