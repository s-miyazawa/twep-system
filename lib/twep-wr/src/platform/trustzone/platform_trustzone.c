                                           ...
                                           twep_wr_owned_bytes_t *out_response_cbor)
{
    ...
    status = twep_tz_read_dev_agent_public_key(ctx, &dev_agent_public_key, &dev_agent_public_key_len);
    if (status != TWEP_WR_OK) {
        twep_tz_free_artifacts(teep_agent_wasm, catalog, app_wasm);
        return status;
    }
    ...
    twep_tz_write_verified_diagnostics(ctx);
    ...
    for (uint32_t round = 0; round < TWEP_WR_TZ_HOST_IO_MAX_ROUNDS; round++) {
        ...
        if (!twep_tz_parse_response(response, op.params[1].tmpref.size,
            ...
            if (verified_acceptance &&
                twep_tz_session_secure_storage_get(
                ...
                (void)twep_tz_session_secure_storage_get(
            ...
            if (have_evidence_snapshot) {
                twep_tz_write_verified_evidence_snapshot(
        ...
        status = twep_tz_perform_host_io(ctx, &need, &host_io_result, &host_io_result_len);
        if (status != TWEP_WR_OK) {
            free(host_io_result);
            twep_tz_close(&session);
            free(response);
        ...
        resume_envelope = twep_tz_make_resume_envelope(need.request_id, host_io_result,
