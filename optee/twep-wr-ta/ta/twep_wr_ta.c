...
static bool wasm_has_import_section(const void *wasm, size_t wasm_len);
static bool ensure_wamr_runtime(void);
					      ...
					      size_t *out_len)
{
	...
	if (!ensure_wamr_runtime())
		return TEE_ERROR_GENERIC;
	if (wasm_has_import_section(app_wasm->ptr, app_wasm->len)) {
		res = TEE_ERROR_BAD_FORMAT;
		goto out;
	}
					  ...
					  const char *label)
{
	...
	if (kind == TWEP_TA_ENVELOPE_EXECUTE && seen.wasm_view.ptr &&
		...
		if (wasm_has_import_section(app_runtime_wasm.ptr,
				      ...
				      TEE_Param params[4])
{
	...
	if (wasm_has_import_section(params[0].memref.buffer, params[0].memref.size)) {
